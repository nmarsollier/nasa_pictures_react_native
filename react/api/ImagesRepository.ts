...
let currentImages = new Map<String, ImageValue[]>()

export function imageUrl(image: ImageValue) {
    return `https://epic.gsfc.nasa.gov/archive/enhanced/${year(image.date)}/${month(image.date)}/${day(image.date)}/png/${image.image}.png`
}
